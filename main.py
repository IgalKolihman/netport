...
class Database:
    ...
    def is_reserved(self, resource: str, value):
        ...
        for client in self.get_all_clients():
            for _ in self.get_client_resources(client, f"{resource}:{value}"):
                return True
    ...
    def get_client_resources(self, client_ip: str, resource_regex: str):
        """Get all the client resources that match the resource regex pattern."""
        return self._db.hscan_iter(client_ip, match=f"{resource_regex}:*")
...
def get_client_resources(client_ip: str, resource: str = "*"):
    return db.get_client_resources(client_ip, resource)
